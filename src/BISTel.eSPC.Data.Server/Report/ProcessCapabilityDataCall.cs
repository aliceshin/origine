...
{
    public class ProcessCapabilityDataCall : DataBase
    {
        #region ::: SELECT

        public DataSet GetModelContext(byte[] baData, bool isATT)
        {
            DataSet ds = new DataSet();
            string strSQL = string.Empty;
            string strWhere = string.Empty;

            //isATT에 따른 변화, KBLEE, Start
            string modelMstTblName = string.Empty;
            string modelConfigTblName = string.Empty;
            string modelContextMstTblName = string.Empty;

            if (isATT)
            {
                modelMstTblName = TABLE.MODEL_ATT_MST_SPC;
                modelConfigTblName = TABLE.MODEL_CONFIG_ATT_MST_SPC;
                modelContextMstTblName = TABLE.MODEL_CONTEXT_ATT_MST_SPC;
            }
            else
            {
                modelMstTblName = TABLE.MODEL_MST_SPC;
                modelConfigTblName = TABLE.MODEL_CONFIG_MST_SPC;
                modelContextMstTblName = TABLE.MODEL_CONTEXT_MST_SPC;
            }
            //isATT에 따른 변화, KBLEE, End
            ...
            try
            {
                ...
                strSQL = string.Format(@"
                 ...
                 @" WHERE MODEL_CONFIG_RAWID IN (SELECT mcms.RAWID
                                                FROM " + modelMstTblName + " mms, " + modelConfigTblName + @" mcms, AREA_MST_PP am    
        ...
        public DataSet GetProcessCapabilityList(byte[] baData, bool isATT)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                try
                {
                    dsResult = GetModelContext(baData, isATT);
                    dt = dsResult.Tables[0].Copy();
                    dt.TableName = modelContextMstTblName;
                    ds.Tables.Add(dt);
                }
                catch
                {
        ...
        public DataSet GetPpkReport(byte[] baData, bool isATT)
        {
            DataSet ds = new DataSet();
            string strSQL = string.Empty;
            string strWhere = string.Empty;
        ...
        /// <returns></returns>
        public DataSet GetDataTRXData(byte[] baData, bool isATT)
        {
            DataSet ds = new DataSet();
