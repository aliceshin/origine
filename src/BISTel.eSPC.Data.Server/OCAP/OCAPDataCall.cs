...
namespace BISTel.eSPC.Data.Server.OCAP
{
    class OCAPDataCall : DataBase
    {
        ...
        {
            string query = "SELECT RAWID, OOC_PROBLEM, OOC_CAUSE, OOC_SOLUTION, OOC_COMMENT, " +
                           "       NVL(FALSE_ALARM_YN, 'N') as FALSE_ALARM_YN " +
                           " FROM OOC_TRX_SPC " +
                           " WHERE RAWID in ({0}) ";
        ...
        public DataSet GetOCAPListData(byte[] baData, bool isATT)
        {
            DataSet ds = new DataSet();
            ...
            try
            {
                ...
                if (sOCAP_OOC == "OCAP")
                {
                    ...
                    if (!isATT)
                    {
                        ...
                        string SQL_GET_FAULT_REPORT_SYSTEM = @"SELECT *
                    ...
                    if (llstData.Contains(Definition.DynamicCondition_Search_key.CHART_ID) && llstData[Definition.DynamicCondition_Search_key.CHART_ID] != null)
                    {
                        if (isComApplied)
                        {
                            sqlQuery = @"SELECT /*+ ORDERED  USE_NL(mms, ots, ortc ) 
                                           ...
                                           mms.default_chart_list, mms.param_type_cd, ots.main_module_id, ots.module_id, 
                                           ots.eqp_id, ots.lot_id, ots.substrate_id, ots.cassette_slot, ots.operation_id, 
                                           ots.product_id, ots.recipe_id, ots.step_id, '' as ocap_rawid_list, 
                                           NVL (ots.false_alarm_yn, 'N') as FALSE_ALARM_YN, ots.module_alias, mms.location_rawid, mms.area_rawid
                        ...
                        else
                        {
                            if (isATT)
                            {
                                sqlQuery = @"SELECT /*+ ORDERED  USE_NL(mms, ots, octs ) 
								                   ...
								                   mms.default_chart_list, ots.main_module_id, ots.module_id, 
                                                   ots.eqp_id, ots.lot_id, ots.substrate_id, ots.cassette_slot, ots.operation_id, 
                                                   ots.product_id, ots.recipe_id, ots.step_id, octs.ocap_rawid_list, 
                                                   NVL (ots.false_alarm_yn, 'N') as FALSE_ALARM_YN, mms.location_rawid
							                  FROM (SELECT /*+ ORDERED   USE_NL( amp, mms, mcms, mcoms ) 
											                   index(mms IDX_MODEL_MST_SPC_UK)  
											                   index(mcms IDX_MODEL_MC_MST_PP_SPC_UK_01) 
											                   index(mcoms IDX_MODEL_CO_MST_SPC_001 ) */
        ...
        public DataSet GetOCAPDetails(byte[] baData, bool isATT)
        {
            DataSet ds = new DataSet();
            string sqlQuery = string.Empty;
            string ocap_rawid = string.Empty;

            //ATT에 따라 달라지는 부분, KBLEE, START
            ...
            try
            {
                ...
                else if (llstData[Definition.DynamicCondition_Condition_key.OCAP_DATATABLE] != null)
                {
                    ...
                    foreach (DataRow dr in dtOCAPRawID.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(ocap_rawid))
                        {
                            ocap_rawid += ",";
                        }
        ...
        //SPC-703
        public DataSet GetOOCCommentList(string OCAPRawid, bool isATT)
        {
            DataSet dsResult = new DataSet();
            ...
            try
            {
                ...
                strSQL = string.Format(@"SELECT location_rawid, area_rawid, ooc_cause 
                                           FROM OOC_CAUSE_MST_SPC 
                                          WHERE location_rawid = {0} AND area_rawid = {1} 
                                          ORDER BY ooc_cause asc ", LocationRawid, AreaRawid);
                ...
                strSQL = string.Format(@"SELECT location_rawid, area_rawid, ooc_comment 
                                           FROM OOC_COMMENT_MST_SPC 
                                          WHERE location_rawid = {0} AND area_rawid = {1} 
                                          ORDER BY ooc_comment asc ", LocationRawid, AreaRawid);
                ...
                strSQL = string.Format(@"SELECT location_rawid, area_rawid, ooc_problem 
                                           FROM OOC_PROBLEM_MST_SPC 
                                          WHERE location_rawid = {0} AND area_rawid = {1} 
                                          ORDER BY ooc_problem ", LocationRawid, AreaRawid);
                ...
                strSQL = string.Format(@"SELECT location_rawid, area_rawid, ooc_solution 
                                           FROM OOC_SOLUTION_MST_SPC 
                                          WHERE location_rawid = {0} AND area_rawid = {1} 
                                          ORDER BY ooc_solution ", LocationRawid, AreaRawid);
        ...
        public DataSet getGroupInfoByChartId(string chartId, bool isATT)
        {
            DataSet ds = new DataSet();
