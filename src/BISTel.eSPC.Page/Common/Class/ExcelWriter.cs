   ...
   public class ExcelWriter : IDisposable
    {
        ...
        public void WriteSheet(string sheetName, DataTable dt)
        {
            // Write the worksheet contents
            WriteStartWorksheet(sheetName);

            //Write header row
            WriteStartRow();
            foreach (DataColumn col in dt.Columns)
                WriteExcelUnstyledCell(col.Caption);
            WriteEndRow();
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                foreach (object o in row.ItemArray)
                {
                    if(o is DBNull)
                    {
                        WriteExcelAutoStyledCell("");
                    }
                    else
                    {
                        WriteExcelAutoStyledCell(o);
                    }
                }
                WriteEndRow();
            }
