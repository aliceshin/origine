...
namespace BISTel.eSPC.Page.Common
{
    public partial class ChartDataPopup : BasePopupFrm
    {
        ...
        public virtual void InitializeBSpread()
        {
            ...
            //SPC-783 by Louisyou ContextMenu추가
            ...
            if (this.isVisibleRawData)
            {
                ...
                //Rawfilter = new MenuItem("Filter");
                //Rawfilter.Click += new EventHandler(Rawfilter_Click);
                //bsprRawData.ContextMenu.MenuItems.Add(Rawfilter);

                //Rawfreeze = new MenuItem("Freeze");
                //Rawfreeze.Click += new EventHandler(Rawfreeze_Click);
                //bsprRawData.ContextMenu.MenuItems.Add(Rawfreeze);
        ...
        //SPC-783 louis you - ContextMenu 추가 ( filter, FilterHide,freeze,unfreeze)
        ...
        {
            if (bsprData.ActiveSheet.ColumnCount <= 0 || bsprRawData.ActiveSheet.ColumnCount <= 0)
                return;
        ...
        private void bbtnList_ButtonClick(string name)
        {
            try
            {
                // SPC-649 Clicking Export-Excel-Button In Chart Data Popup, We Can find both same data in raw data and chart data.- 2011.10.29 by ANDREW KO
