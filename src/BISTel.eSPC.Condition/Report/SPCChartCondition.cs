...
namespace BISTel.eSPC.Condition.Report
{
    ...
    public partial class SPCChartCondition : ADynamicCondition
    {
        ...
        public override LinkedList GetParameter(LinkedList ll)
        {
            ...
            ll.Add(Definition.DynamicCondition_Search_key.CHART_ID, PROC_CreateChartID());
            ll.Add(Definition.DynamicCondition_Search_key.RESTRICT_CONDITION, PROC_CreateRestrictCondition());
        ...
        private void SetCondition(LinkedList ll)
        {
            ...
            if (_condition.Contains(Definition.DynamicCondition_Search_key.DATETIME_FROM))            
            {
                ...
                this.mStartTime = DCUtil.GetValueData(dt);                       
            }
            ...
            if (_condition.Contains(Definition.DynamicCondition_Search_key.DATETIME_TO))
            {
                ...
                this.mEndTime = DCUtil.GetValueData(dt);                         
            }
            ...
            if (_condition.Contains(Definition.DynamicCondition_Search_key.CHART_ID))
            {
                dt = (DataTable)_condition[Definition.DynamicCondition_Search_key.CHART_ID];
                if (dt.Rows.Count > 0)
                {
                    this.btxt_ConfigRawID.Text = dt.Rows[0][Definition.DynamicCondition_Condition_key.DISPLAYDATA].ToString();
                }
            }

            if (_condition.Contains(Definition.DynamicCondition_Search_key.RESTRICT_CONDITION))
            {
                dt = (DataTable)_condition[Definition.DynamicCondition_Search_key.RESTRICT_CONDITION];
                if (dt.Rows.Count > 0)
                {
                    bool bResult = Convert.ToBoolean(dt.Rows[0][Definition.DynamicCondition_Condition_key.DISPLAYDATA].ToString());
                    this.bckRestrict.Checked = bResult;
                }
            }
        ...
        private DataTable PROC_CreateRestrictCondition()
        {
            DataTable dt = new DataTable();
            dt.Columns.Add(Definition.DynamicCondition_Condition_key.VALUEDATA, typeof(string));
            dt.Columns.Add(Definition.DynamicCondition_Condition_key.DISPLAYDATA, typeof(string));
            ...
            DataRow dr = dt.NewRow();
            dr[Definition.DynamicCondition_Condition_key.VALUEDATA] = bckRestrict.Checked.ToString();
            dr[Definition.DynamicCondition_Condition_key.DISPLAYDATA] = bckRestrict.Checked.ToString();
            dt.Rows.Add(dr);
        ...
        private DataTable PROC_CreateChartID()
        {
            DataTable dt = new DataTable();
            ...
            dr[Definition.DynamicCondition_Condition_key.DISPLAYDATA] = this.btxt_ConfigRawID.Text.Trim();
            dt.Rows.Add(dr);
            return dt;
        }
        ...
        private void PROC_ContextCall()
        {
            ...
            try{                        
                ...
                {
                    _dsContext = this._wsSPC.GetATTSPCContextList(llstData.GetSerialData());
                }
                else if (this.GetType().FullName == "BISTel.eSPC.Condition.Report.SPCChartCondition")
                {
                    _dsContext = this._wsSPC.GetSPCContextList(llstData.GetSerialData());
                }
