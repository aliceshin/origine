...
namespace BISTel.eSPC.Common
{
            ...
            */
            ...
            else
            {
                ...
                foreach (FieldInfo Field in GroupByFieldInfo)
                {
                    ...
                    if(!dt.Columns.Contains(Field.FieldAlias))
                    {
                        if (Field.Aggregate == null)
                        {
                            if(dc==null)
                                dt.Columns.Add(Field.FieldAlias, typeof(string));
                            else
    ...
    string RowFilter, string GroupBy)
                ...
                foreach (FieldInfo Field in GroupByFieldInfo)
                {
                    switch (Field.Aggregate)    //this test is case-sensitive
                    {
                        ...
                        case "min":
                            ...
                            break;
                        case "round1":
                            DestRow[Field.FieldAlias] = Round(DestRow[Field.FieldAlias], 1);
                            break;
                        case "round2":
                            DestRow[Field.FieldAlias] = Round(DestRow[Field.FieldAlias], 2);                            
                            break;                                                      
                    }
                }
        ...
        private object Round(object a, int iDecimals)
        {
            ...
            {
                return Math.Round(ParseDecimal(a), iDecimals);
            }
        }
        ...
        private decimal ParseDecimal(object a)
        {
            return string.IsNullOrEmpty(a.ToString()) ?0:decimal.Parse(a.ToString());
        }
