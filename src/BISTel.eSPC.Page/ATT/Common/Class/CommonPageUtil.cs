...
namespace BISTel.eSPC.Page.ATT.Common
{
    public class CommonPageUtil
    {
        ...
        public static LinkedList SetContextType( DataSet argDataSet)
        {
            ...
            {
                contextTypeInfo = new SPCStruct.ContextTypeInfo(dr[BISTel.eSPC.Common.COLUMN.CODE].ToString(), dr[BISTel.eSPC.Common.COLUMN.NAME].ToString());                
                _llst.Add(dr[BISTel.eSPC.Common.COLUMN.CODE].ToString(), contextTypeInfo);
            }
            return _llst;
        }
                    ...
                    else if (sTableName == Definition.TableName.USERNAME_TEMPDATA)
                    {
                        ...
                        if (!DataUtil.IsNullOrEmptyDataSet(_dsSelect)){                            
                            ...
                            if (bResultTable)
                            {
                                ...
                                {
                                    if (dc.ColumnName.Contains(BISTel.eSPC.Common.COLUMN.DATA_LIST) || dc.ColumnName.Contains(BISTel.eSPC.Common.COLUMN.CONTEXT_LIST) || dc.ColumnName.Contains("NORMAL") || dc.ColumnName.Contains("ORIGINAL_PARAM")) continue;                                               
                            ...
                            {
                                dtResult.Merge(dt);
                                if (dtResult.Columns.Contains(BISTel.eSPC.Common.COLUMN.DATA_LIST)) dtResult.Columns.Remove(BISTel.eSPC.Common.COLUMN.DATA_LIST);
                                if (dtResult.Columns.Contains(BISTel.eSPC.Common.COLUMN.CONTEXT_LIST)) dtResult.Columns.Remove(BISTel.eSPC.Common.COLUMN.CONTEXT_LIST);
                            }
                        }
                    }
                }
        ...
        public static DataTable ExcelExport(DataTable dt, List<string> _lstRawColumn)
        {
            ...
            for (int i = dt.Columns.Count - 1; i >= 0; i--)
            {
                ...
                if ((sCol.IndexOf("_RULE") > -1)
                    ...
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.DEFAULT_CHART_LIST) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.DATA_LIST) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.CONTEXT_LIST) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.CONTEXT_KEY) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.FILE_DATA) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.RESTRICT_SAMPLE_DAYS) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.RESTRICT_SAMPLE_COUNT) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.COMPLEX_YN) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.MAIN_YN) > -1)
                    || (sCol.IndexOf(Definition.COL_TOGGLE_YN) > -1)
                    || (sCol.IndexOf(Definition.COL_TOGGLE) > -1)
                    || (sCol.IndexOf("NORMAL") > -1)
                    ...
                    || (sCol.IndexOf(Definition.CHART_COLUMN.RESTRICT_SAMPLE_HOURS) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.RAWID) > -1)
                   )
                {
        ...
        public static DataTable ExcelExportRaw(DataTable dt, List<string> _lstRawColumn)
        {
            ...
            for (int i = dt.Columns.Count - 1; i >= 0; i--)
            {
                ...
                if ((sCol.IndexOf("_RULE") > -1)
                    ...
                    || (sCol.IndexOf(Definition.COL_TOGGLE_YN) > -1)
                    || (sCol.IndexOf(Definition.COL_TOGGLE) > -1)
                    || (sCol.IndexOf(BISTel.eSPC.Common.COLUMN.SAMPLE_COUNT) > -1)
                    || (sCol.IndexOf(Definition.CHART_COLUMN.ORDERINFILEDATA) > -1)
                    || (sCol.IndexOf(Definition.CHART_COLUMN.TABLENAME) > -1)
                    || (sCol.IndexOf(Definition.CHART_COLUMN.DTSOURCEID) > -1)
                   )
